  ...
  return (
    <section className="py-20 bg-white border-b border-surface-border">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Header */}
        <div 
          className="max-w-3xl mb-12 md:mb-16 reveal-init"
        ...
        {/* 3 Root Causes Cards (with CSS scroll-snap & staggered reveal) */}
        <div className="flex md:grid md:grid-cols-3 gap-6 overflow-x-auto snap-x snap-mandatory pb-4 md:pb-0 -mx-4 px-4 md:mx-0 md:px-0">
          {problems.map((item, idx) => (
            <div
              key={idx}
              className="snap-center shrink-0 w-[85vw] max-w-[320px] md:w-auto md:max-w-none p-8 rounded-2xl bg-surface-canvas border border-surface-border hover:border-slate-300 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between reveal-init group"
              style={{ "--reveal-delay": `${(idx + 1) * 150}ms` } as React.CSSProperties}
            >
              <div>
                <div className={`w-12 h-12 rounded-xl ${item.badgeColor} flex items-center justify-center font-bold mb-6 group-hover:scale-110 transition-transform`}>
                  {item.icon}
                </div>
              ...
              <div className={`text-xs font-semibold ${item.effectColor} flex items-center gap-1 mt-auto pt-2 border-t border-slate-200/60`}>
